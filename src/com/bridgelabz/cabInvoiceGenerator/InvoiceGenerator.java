...
public class InvoiceGenerator {
    ...
    public InVoiceDescription calculateFareDescription(Ride[] rides, boolean normalRide) {
        double cost = 0;
        for (Ride ride : rides) {
            cost = cost + this.calculateFareForRideType(ride.distance, ride.time, ride.rideType);
        }
        return new InVoiceDescription(rides.length, cost, normalRide);
    }

    public InVoiceDescription getInvoiceDescription(String userId, boolean normalRide) {
